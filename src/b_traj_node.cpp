...
int _max_x_id, _max_y_id, _max_z_id, _max_local_x_id, _max_local_y_id,
    _max_local_z_id;
int _traj_id = 1;
...
ros::Publisher _fm_path_vis_pub, _local_map_vis_pub, _inf_map_vis_pub,
    _corridor_vis_pub, _traj_vis_pub, _grid_path_vis_pub, _nodes_vis_pub,
    _traj_pub, _checkTraj_vis_pub, _stopTraj_vis_pub;
...
VectorXd getStateFromBezier(const MatrixXd &polyCoeff, double t_now,
                            int seg_now);
...
void rcvPointCloudCallBack(const sensor_msgs::PointCloud2 &pointcloud_map) {
  ...
  double local_c_x =
      (int)((_start_pt(0) - _x_local_size / 2.0) * _inv_resolution + 0.5) *
      _resolution;
  double local_c_y =
      (int)((_start_pt(1) - _y_local_size / 2.0) * _inv_resolution + 0.5) *
      _resolution;
  double local_c_z =
      (int)((_start_pt(2) - _z_local_size / 2.0) * _inv_resolution + 0.5) *
      _resolution;

  _local_origin << local_c_x, local_c_y, local_c_z;

  Translation3d origin_local_translation(_local_origin(0), _local_origin(1),
                                         _local_origin(2));
  Quaterniond origin_local_rotation(1.0, 0.0, 0.0, 0.0);

  Affine3d origin_local_transform =
      origin_local_translation * origin_local_rotation;
...
bool checkExecTraj() {
  ...
  _stop_traj_vis.type = _check_traj_vis.type =
      visualization_msgs::Marker::SPHERE_LIST;
  _stop_traj_vis.action = _check_traj_vis.action =
...
bool checkCoordObs(Vector3d checkPt) {
  if (collision_map->Get(checkPt(0), checkPt(1), checkPt(2)).first.occupancy >
      0.0)
    return true;
...
pair<Cube, bool> inflateCube(Cube cube, Cube lstcube) {
  ...
  for (int i = 0; i < 8; i++) {
    ...
    if (collision_map
            ->Get((int64_t)pt_idx(0), (int64_t)pt_idx(1), (int64_t)pt_idx(2))
            .first.occupancy > 0.5) {
  ...
  */
  ...
  while (iter < _max_inflate_iter) {
    ...
    for (id_y = vertex_idx(0, 1); id_y >= y_lo; id_y--) {
      ...
      for (id_x = vertex_idx(0, 0); id_x >= vertex_idx(3, 0); id_x--) {
        ...
        for (id_z = vertex_idx(0, 2); id_z >= vertex_idx(4, 2); id_z--) {
          double occupy =
              collision_map->Get((int64_t)id_x, (int64_t)id_y, (int64_t)id_z)
                  .first.occupancy;
    ...
    if (collide) {
      ...
      vertex_idx(0, 1) = vertex_idx(3, 1) = vertex_idx(7, 1) =
          vertex_idx(4, 1) = id_y + 1;
    ...
    if (collide) {
      ...
      vertex_idx(1, 1) = vertex_idx(2, 1) = vertex_idx(6, 1) =
          vertex_idx(5, 1) = id_y - 1;
    ...
    if (collide) {
      ...
      vertex_idx(0, 0) = vertex_idx(1, 0) = vertex_idx(5, 0) =
          vertex_idx(4, 0) = id_x - 1;
    ...
    if (collide) {
      ...
      vertex_idx(3, 0) = vertex_idx(2, 0) = vertex_idx(6, 0) =
          vertex_idx(7, 0) = id_x + 1;
    ...
    vertex_idx(0, 2) = vertex_idx(1, 2) = vertex_idx(2, 2) = vertex_idx(3, 2) =
        id_z - 1;
    ...
    if (collide) {
      ...
      vertex_idx(4, 2) = vertex_idx(5, 2) = vertex_idx(6, 2) =
          vertex_idx(7, 2) = id_z + 1;
...
bool isContains(Cube cube1, Cube cube2) {
  if (cube1.vertex(0, 0) >= cube2.vertex(0, 0) &&
      ...
      cube1.vertex(0, 2) >= cube2.vertex(0, 2) &&
      cube1.vertex(6, 0) <= cube2.vertex(6, 0) &&
      cube1.vertex(6, 1) >= cube2.vertex(6, 1) &&
      cube1.vertex(6, 2) <= cube2.vertex(6, 2))
    return true;
  else
    return false;
}
...
void trajPlanning() {
  ...
  if (_is_use_fm) {
      ...
      for (unsigned int j = 0; j < size_y; j++) {
        for (unsigned int i = 0; i < size_x; i++) {
          ...
          if (collision_map_local->Inside(index)) {
            double d = sqrt(EDT.GetImmutable(index).first.distance_square) *
                       _resolution;
            flow_vel = velMapping(d, max_vel);
          } else
            flow_vel = max_vel;

          if (k == 0 || k == (size_z - 1) || j == 0 || j == (size_y - 1) ||
              i == 0 || i == (size_x - 1))
            flow_vel = 0.0;
    ...
    Coord3D goal_point = {(unsigned int)startIdx3d[0],
                          (unsigned int)startIdx3d[1],
                          (unsigned int)startIdx3d[2]};
    Coord3D init_point = {(unsigned int)endIdx3d[0], (unsigned int)endIdx3d[1],
                          (unsigned int)endIdx3d[2]};
    ...
    Solver<FMGrid3D> *fm_solver =
        new FMMStar<FMGrid3D>("FMM*_Dist", TIME); // LSM, FMM
    ...
    ROS_WARN("[Fast Marching Node] Time in Fast Marching computing is %f",
             (time_aft_fm - time_bef_fm).toSec());
    ...
    if (grad3D.gradient_descent(grid_fmm, goalIdx, path3D, path_vels, time) ==
        -1) {
    ...
    for (int i = 0; i < (int)path3D.size(); i++) {
      coord_x =
          max(min((path3D[i][0] + 0.5) * _resolution + _map_origin(0), _x_size),
              -_x_size);
      coord_y =
          max(min((path3D[i][1] + 0.5) * _resolution + _map_origin(1), _y_size),
              -_y_size);
    ...
    ROS_WARN("Time consume in corridor generation is %f",
             (time_aft_corridor - time_bef_corridor).toSec());
  ...
  if (_trajectoryGenerator.BezierPloyCoeffGeneration(
          corridor, _MQM, pos, vel, acc, _MAX_Vel, _MAX_Acc, _traj_order,
          _minimize_order, _cube_margin, _is_limit_vel, _is_limit_acc, obj,
          _bezier_coeff) == -1) {
...
void timeAllocation(vector<Cube> &corridor, vector<double> time) {
  ...
  for (int i = 0; i < (int)points.size() - 1; i++) {
    ...
    if (dtxyz < tmp_time[i] * 0.5)
      tmp_time[i] = dtxyz; // if FM given time in this segment is rediculous
                           // long, use the new value
  }
...
int main(int argc, char **argv) {
  ...
  _inf_map_vis_pub =
      nh.advertise<sensor_msgs::PointCloud2>("vis_map_inflate", 1);
  _local_map_vis_pub =
      nh.advertise<sensor_msgs::PointCloud2>("vis_map_local", 1);
  _traj_vis_pub = nh.advertise<visualization_msgs::Marker>("trajectory_vis", 1);
  _corridor_vis_pub =
      nh.advertise<visualization_msgs::MarkerArray>("corridor_vis", 1);
  _fm_path_vis_pub =
      nh.advertise<visualization_msgs::MarkerArray>("path_vis", 1);
  _grid_path_vis_pub =
      nh.advertise<visualization_msgs::MarkerArray>("grid_path_vis", 1);
  _nodes_vis_pub =
      nh.advertise<visualization_msgs::Marker>("expanded_nodes_vis", 1);
  _checkTraj_vis_pub =
      nh.advertise<visualization_msgs::Marker>("check_trajectory", 1);
  _stopTraj_vis_pub =
      nh.advertise<visualization_msgs::Marker>("stop_trajectory", 1);

  _traj_pub =
      nh.advertise<quadrotor_msgs::PolynomialTrajectory>("trajectory", 10);
...
Vector3d getPosFromBezier(const MatrixXd &polyCoeff, double t_now,
                          int seg_now) {
    ...
    for (int j = 0; j < ctrl_num1D; j++) {
      ...
      if (j < ctrl_num1D - 2)
        ret[i + 6] += _Ca(j) * _traj_order * (_traj_order - 1) *
                      (ctrl_now(i * ctrl_num1D + j + 2) -
                       2 * ctrl_now(i * ctrl_num1D + j + 1) +
                       ctrl_now(i * ctrl_num1D + j)) *
                      pow(t_now, j) * pow((1 - t_now), (_traj_order - j - 2));

      if (j < ctrl_num1D - 3)
        ret[i + 9] += _Cj(j) * _traj_order * (_traj_order - 1) *
                      (_traj_order - 2) *
                      (ctrl_now(i * ctrl_num1D + j + 3) -
                       3 * ctrl_now(i * ctrl_num1D + j + 2) +
                       3 * ctrl_now(i * ctrl_num1D + j + 1) -
                       ctrl_now(i * ctrl_num1D + j)) *
...
void visCorridor(vector<Cube> corridor) {
  ...
  for (int i = 0; i < int(corridor.size()); i++) {
    ...
    mk.pose.position.x =
        (corridor[i].vertex(0, 0) + corridor[i].vertex(3, 0)) / 2.0;
    mk.pose.position.y =
        (corridor[i].vertex(0, 1) + corridor[i].vertex(1, 1)) / 2.0;

    if (_is_proj_cube)
      mk.pose.position.z = 0.0;
    else
      mk.pose.position.z =
          (corridor[i].vertex(0, 2) + corridor[i].vertex(4, 2)) / 2.0;
