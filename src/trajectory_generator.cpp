...
{
// whether or not adding extra constraints for ensuring the velocity
// feasibility
#define ENFORCE_VEL isLimitVel
// whether or not adding extra constraints for ensuring the acceleration
// feasibility
#define ENFORCE_ACC isLimitAcc
  ...
  // p, v, a in x, y, z axis in each
  // segment's joint position
  // 速度约束数量(每一段速度只有 traj_order 个控制点)
  int vel_con_num = 3 * traj_order * segment_num;
  // 加速度约束数量(每一段加速度只有 traj_order-1 个控制点)
  ...
  // x_var用于存储优化过程中计算出的控制点变量
  double x_var[ctrlP_num];
  // primalobj用于存储优化的最终目标值
  double primalobj;
  ...
  for (int k = 0; k < segment_num; k++) {
    ...
    for (int i = 0; i < 3; i++) {
      for (int j = 0; j < n_poly; j++) {
        ...
        vb_x = make_pair(
            MSK_BK_RA,
            make_pair(lo_bound, up_bound)); // # vb_x means: varialbles boundary
                                            // of unknowns x (Polynomial coeff)
